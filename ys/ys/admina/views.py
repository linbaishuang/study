...
def update_admin(req):
    ...
    if req.method == "GET":
        ...
        Administrators = models.Administrators.objects.filter(username=username)[0]
        department = models.Department.objects.filter(departmentId=Administrators.departmentId_id)[0]
        return render_to_response("admina/add_admin.html", {
            "name": Administrators.name, "username": Administrators.username,
            "password": Administrators.password, "department": department.name,
            "id": Administrators.username
        })
        ...
        department = req.POST["department"]
        Administrators = models.Administrators.objects.filter(username=username)[0]
        models.Department.objects.filter(departmentId=Administrators.departmentId_id).update(name=department)
        departments = models.Department.objects.filter(departmentId=Administrators.departmentId_id)
        models.Administrators.objects.filter(username=username).update(
            name=name,
            password=password,
            departmentId_id=departments[0].departmentId
        )
        return HttpResponse(1)

@csrf_exempt
def update_colum(req):
    '''
    修改栏目信息
    :param req: 
    :return: 
    ...
    if req.method == "GET":
        ...
        return render_to_response("admina/col_page1.html", {
            ...
            "level": column.level, "parentColum": column.parentColum,
            "sort": column.sort
        })
    elif req.method == "POST":
        pass
